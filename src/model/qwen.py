...
def _maybe_bnb_config(args):
    """Return a BitsAndBytesConfig if quantization flags are enabled, else None."""
    ...
    try:
        ...
        raise RuntimeError(
            "bitsandbytes quantization requested, but transformers BitsAndBytesConfig is unavailable. "
            "Install bitsandbytes and a recent transformers."
        ) from e
    import torch
...
def load_model(args, model_name_or_path, memory_for_model_activations_in_gb=2, peft_path=None):
    ...
    qconfig = _maybe_bnb_config(args)
